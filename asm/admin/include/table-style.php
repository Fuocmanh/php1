...
if (isset($_POST['search'])) :
    $key = $_POST['key'];
    $sql = "SELECT * FROM `check` WHERE CONCAT(style_id,Style) LIKE '%$key%' ORDER BY style_id LIMIT $start_from, $limit";
    $query_pro = "SELECT COUNT(style_id) AS member FROM `check` WHERE CONCAT(style_id,Style) LIKE '%$key%'";
else :
                        ...
                        Denied
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center space-x-4 text-sm">
                                    <a href="module/pages/edit-style.php?id=<?= $style['style_id']; ?>" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Edit">
