...
?>
    ...
    <head>
        ...
        <link rel="stylesheet" href="../assets/client/css/details.css">
    ...
    <body>
        ...
        <?php
        $id = $_GET['id'];
        $sql = "SELECT * FROM `products` p  WHERE pro_id='$id'";
        $pros = $conn->query($sql);
        if ($pros->num_rows > 0) :
        ...
        ?>
                <div class="boxx" id="form1">
                    <div class="left">
                        <div class="img_top">
                            <img src="<?= '../admin/uploads/' . $pro['image'] ?>" alt="">
                        </div>
                        <?php
                        $img = "SELECT * FROM `img_pro`  WHERE pro_id='$id'";
                        $imgs = $conn->query($img);
                        if ($imgs->num_rows > 0) :
                            while ($img = $imgs->fetch_assoc()) :
                        ?>
                                <div class="img_bot">
                                    <img src="<?= '../admin/uploads/' . $img['img1'] ?>" alt="">
                                    <img src="<?= '../admin/uploads/' . $img['img2'] ?>" alt="">
                                    <img src="<?= '../admin/uploads/' . $img['img3'] ?>" alt="">
                                    <img src="<?= '../admin/uploads/' . $img['img4'] ?>" alt="">
                                </div>
                        <?php
                            endwhile;
                        endif;
                                ...
                                <div id="panelsStayOpen-collapseOne" class="accordion-collapse collapse show" aria-labelledby="panelsStayOpen-headingOne">
                                    <div class="accordion-body">
                                        <div class="box1">
                                            <div class="left">
                                                <?php
                                                $det = "SELECT * FROM `details`  WHERE pro_id='$id'";
                                                $dets = $conn->query($det);
                                                if ($dets->num_rows > 0) :
                                                    while ($det = $dets->fetch_assoc()) :
                                                ?>
                                                        <h2><?= $det['h2'] ?></h2>
                                                        <p><?= $det['details'] ?></p>
                                                <?php
                        ...
                        <div class="all" style="top: -300.062px;">
                            <div class="alll">
                                ...
                                <div>
                                    ...
                                    <div class="gl-vspace-bpall-small"><?php
                                                                        $det = "SELECT * FROM `details`  WHERE pro_id='$id'";
                                                                        $dets = $conn->query($det);
                                                                        if ($dets->num_rows > 0) :
                                                                            while ($det = $dets->fetch_assoc()) :
                            ...
                            <div>
                                ...
                                <div id="add-to-bag" tabindex="-1" class="add-to-bag">
                                    ...
                                    <input type="hidden" name="id_sp" id="" value="<?= $pro['pro_id'] ?>">
                                    <button type="submit" class="buttom-bannerr" data-auto-id="add-to-bag" name="addcart" title="Add To Bag"><span class="add">Add To Bag <img src="../assets/client/images/right-arrow.png" alt=""></span><svg class="gl-icon gl-cta__icon">
                                            <use xlink:href="#arrow-right-long"></use>
                                        </svg></button>
                                    </form>
                                    <div class="wish"><button><img src="../assets/client/images/favorite.png"></button></div>
                                </div>
                            </div>
                        </div>
                    </div>
